...
[[ -z $runtimes ]] && runtimes=$(cat $runtime_cfg | jq '.runtimeOptions.framework.name' | tr -d '"')
for runtime in $runtimes; do
  ...
  [[ -z $runtime_version ]] && runtime_version=$(cat $runtime_cfg | jq '.runtimeOptions.framework? | select(.name=="'$runtime'") | .version' | tr -d '"')
